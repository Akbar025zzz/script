...
local RE = game:GetService("ReplicatedStorage").Packages._Index["sleitnick_knit@1.7.0"].knit.Services.FishingService.RE

local currentFishData = nil

-- simpan fishData
RE.FishCaught.OnClientEvent:Connect(function(fishData)
    currentFishData = fishData
    print("[FishCaught]", fishData.fish, fishData.rarity)
end)

head.ChildAdded:Connect(function(child)
    if child.Name == "FishingExclamation" then
        print("[!] Tanda seru muncul — ikan nyangkut!")
        task.wait(4)

        if currentFishData then
            local stats = {
                yanksCompleted = math.random(2, 5),
                minigameDurationMs = math.random(13000, 15000),
                redBarDangerTicks = math.random(40, 100),
                snagAttempts = 0,
                snagSuccesses = 0,
                maxRedBarProximity = math.random(60, 95) / 100,
                totalSpamPoints = math.random(20, 50)
            }

            RE.FishingSuccess:FireServer(true, stats)
            firesignal(RE.Cleanup.OnClientEvent)
            firesignal(RE.SetRodLock.OnClientEvent, false)

            print("[Done]", currentFishData.fish, "|", currentFishData.rarity, "| $"..currentFishData.sellPrice)
            currentFishData = nil
        else
            print("[!] FishData kosong!")
        end
    end
end)
...
local EmbeddedEventResolver = (function()
    ...
    local function scanPairs(netFolder)
        ...
        while i <= #children do
            ...
            if nextChild then
                ...
                if currClass == nextClass and not isHash(currName) and isHash(nextName) then
            ...
            if not isHash(name) then
                if curr:IsA("RemoteFunction") and not self._rf[name] then
                    self._rf[name] = curr
                elseif (curr:IsA("RemoteEvent") or curr:IsA("UnreliableRemoteEvent")) and not self._re[name] then
                    self._re[name] = curr
                end
            end
            i = i + 1
    ...
    function self:IsInitialized() return self._initialized end
    function self:Reset()
...
InstantSection:Input({
    ...
    Callback = function(input)
        ...
        if num then InstantV1.Settings.CompleteDelay = num end
    end
})
...
InstantSection:Toggle({
    ...
    Callback = function(state)
        if state then InstantV1.Start() else InstantV1.Stop() end
    end
})
...
CombinedModules.legit = (function()
    ...
    Legit.Settings = { CastWait = 0.5, ClickWait = 0.04, ShakeDelay = 0.05 }
    ...
    local function resetFishingState()
        ...
        pcall(function()
            if NetEvents and NetEvents.RF_CancelFishingInputs then NetEvents.RF_CancelFishingInputs:InvokeServer() end
        end)
    end

    local function mainLoop()
        if not initFishingController() then Legit.Active = false return end
        while Legit.Active do
            local ok, err = pcall(function()
                if FishingController:OnCooldown() then task.wait(0.2) return end
                if FishingController:GetCurrentGUID() then
                    FishingController:RequestFishingMinigameClick()
                    task.wait(Legit.Settings.ClickWait)
                else
                    FishingController:RequestChargeFishingRod(getViewportCenter(), true)
                    task.wait(Legit.Settings.CastWait)
                end
            end)
            if not ok then warn("[Legit] Error: " .. tostring(err)) task.wait(1) end
        end
    end

    local function shakeLoop()
        if not initFishingController() then Legit.AutoShake = false return end
...
LegitSection:Input({
    ...
    Callback = function(input)
        local num = tonumber(input)
        if num then CombinedModules.legit.Settings.CastWait = num end
    end
})
...
LegitSection:Toggle({
    ...
    Value = false,
    Callback = function(state)
        if state then CombinedModules.legit.Start() else CombinedModules.legit.Stop() end
    end
})
    ...
    local function loop()
        while Blatant.Active do
            ...
            if Blatant.Settings.InstantComplete then
                ...
                safeFire(function() local rf = NetEvents.RE_FishingCompleted if rf then rf:FireServer() end end)
                task.wait(0.01)
...
BlatantSection:Input({
    ...
    Callback = function(input)
        local num = tonumber(input)
        if num then BlatantV1.Settings.SpamCastDelay = num end
    end
})
...
BlatantSection:Input({
    ...
    Callback = function(input)
        local num = tonumber(input)
        if num then BlatantV1.Settings.CompleteDelay = num end
    end
})
...
BlatantSection:Toggle({
    ...
    Value = false,
    Callback = function(state)
        if state then BlatantV1.Start() else BlatantV1.Stop() end
    end
})
...
local NoFishingAnimation = (function()
    ...
    M.Enabled = false
    M.Connection = nil
    function M.StartWithDelay() task.wait(0.5) M.Start() end
    function M.Start()
...
SupportSection:Toggle({
    ...
    Value = false,
    Callback = function(state)
        if state then NoFishingAnimation.Start() else NoFishingAnimation.Stop() end
    end
})
...
SupportSection:Toggle({
    ...
    Value = false,
    Callback = function(state)
        if state then AutoEquipRod.Start() else AutoEquipRod.Stop() end
    end
})
...
SupportSection:Toggle({
    ...
    Value = false,
    Callback = function(state)
        if state then LockPosition.Start() else LockPosition.Stop() end
    end
})
...
SupportSection:Toggle({
    ...
    Value = false,
    Callback = function(state)
        if state then DisableCutscenes.Start() else DisableCutscenes.Stop() end
    end
})
...
SupportSection:Toggle({
    ...
    Value = false,
    Callback = function(state)
        if state then DisableExtras.StartSmallNotification() else DisableExtras.StopSmallNotification() end
    end
})
...
SupportSection:Toggle({
    ...
    Value = false,
    Callback = function(state)
        if state then DisableExtras.StartSkinEffect() else DisableExtras.StopSkinEffect() end
    end
})
...
SupportSection:Toggle({
    ...
    Value = false,
    Callback = function(state)
        if state then StableResult.Start() else StableResult.Stop() end
    end
})
...
SupportSection:Toggle({
    ...
    Value = false,
    Callback = function(state)
        if state then WalkOnWater.Start() else WalkOnWater.Stop() end
    end
})
...
CombinedModules.AutoFavorite = (function()
    ...
    local function InitializeReferences()
        ...
        local success = pcall(function()
            ...
            v6 = { Events = { REFav = NetEvents.RE_FavoriteItem } }
            v7 = {
    ...
    local v8 = { selectedName = {}, selectedRarity = {}, selectedVariant = {}, autoFavEnabled = false }
        ...
        pcall(function()
            ...
            for _, item in ipairs(inventory) do
                ...
                if hasNameFilter then
                    if not fishName or not v8.selectedName[fishName] then shouldFavorite = false end
                end
                if shouldFavorite and hasVariantFilter then
                    if variantId == "None" or not v8.selectedVariant[variantId] then shouldFavorite = false end
                end
                if shouldFavorite and hasRarityFilter then
                    if not tierName or not v8.selectedRarity[tierName] then shouldFavorite = false end
                end
    ...
    function AutoFavoriteModule.IsEnabled() return v8.autoFavEnabled end
...
if CombinedModules.AutoFavorite then
    local nameDropdown = AutoFavSection:Dropdown({
        ...
        Callback = function(selected)
            if type(selected) == "table" then CombinedModules.AutoFavorite.SetSelectedNames(selected) end
        end,
    })
    ...
    local variantDropdown = AutoFavSection:Dropdown({
        ...
        Multi = true,
        Callback = function(selected)
            if type(selected) == "table" then CombinedModules.AutoFavorite.SetSelectedVariants(selected) end
        end,
    })
    ...
    AutoFavSection:Dropdown({
        ...
        Multi = true,
        Callback = function(selected)
            if type(selected) == "table" then CombinedModules.AutoFavorite.SetSelectedRarity(selected) end
        end,
    })
    ...
    AutoFavSection:Toggle({
        ...
        Value = false,
        Callback = function(state)
            if state then CombinedModules.AutoFavorite.Start() else CombinedModules.AutoFavorite.Stop() end
        end,
    })
end
...
CombinedModules.AutoEvent = (function()
    ...
    local function applyOffset(v) return Vector3.new(v.X, v.Y + module.HeightOffset, v.Z) end
    local function safeCharacter() return game.Players.LocalPlayer.Character or game.Players.LocalPlayer.CharacterAdded:Wait() end
    ...
    local function doTeleport(pos)
        ...
        local ok = pcall(function()
            ...
            if c.PrimaryPart then c:PivotTo(CFrame.new(pos)) else hrp.CFrame = CFrame.new(pos) end
            hrp.Anchored = false
    ...
    local function IsEventAlive(obj)
        ...
        local success = pcall(function() return obj.Parent ~= nil and obj:IsDescendantOf(workspace) end)
        return success
    end
    local function SearchInAllProps(eventName)
        ...
        for _, child in ipairs(workspace:GetChildren()) do
            if child.Name == "Props" and child:IsA("Model") then table.insert(allProps, child) end
        end
    ...
    local function scanEvent(eventName)
        ...
        if now - lastScanTime < module.ScanCooldown then
            if cachedEventPosition and cachedEventObject and IsEventAlive(cachedEventObject) then return cachedEventPosition end
        end
        lastScanTime = now
        ...
        if coords and #coords > 0 then
            for _, coord in ipairs(coords) do
                local region = Region3.new(coord - Vector3.new(30, 30, 30), coord + Vector3.new(30, 30, 30)):ExpandToGrid(4)
                local ok, parts = pcall(function() return workspace:FindPartsInRegion3(region, nil, 50) end)
    ...
    function module.HasEventPattern(eventName) return EventSearchPatterns[eventName] ~= nil end
    return module
end)()
...
local islandNames = {}
for name, _ in pairs(TeleportModule.Locations) do table.insert(islandNames, name) end
...
IslandSection:Dropdown({
    ...
    Values = islandNames,
    Value = 1,
    Callback = function(selected) selectedIsland = selected end,
})
...
local AutoSellSystem = (function()
    ...
    M.Timer = {}
    M.Count = {}
    local function getSellRemote() return EmbeddedEventResolver:GetRF("SellAllItems") end
    ...
    function M.Timer.Start(interval)
        ...
        if not getSellRemote() then return false end
        if interval and tonumber(interval) and tonumber(interval) >= 1 then timerMode.interval = tonumber(interval) end
    ...
    function M.Timer.SetInterval(seconds)
        if tonumber(seconds) and tonumber(seconds) >= 1 then timerMode.interval = tonumber(seconds) return true end
        return false
    end
    function M.Count.Start(target)
        if countMode.enabled then return false end
        if not getSellRemote() then return false end
        if target and tonumber(target) and tonumber(target) > 0 then countMode.target = tonumber(target) end
    ...
    function M.Count.SetTarget(count)
        if tonumber(count) and tonumber(count) > 0 then countMode.target = tonumber(count) return true end
        return false
    end
    return M
end)()
...
local AutoBuyWeather = (function()
    ...
    local selected = {}
    local function getWeatherRemote() return EmbeddedEventResolver:GetRF("PurchaseWeatherEvent") end
    M.AllWeathers = { "Cloudy", "Storm", "Wind", "Snow", "Radiant", "Shark Hunt" }
    function M.SetSelected(list) selected = list or {} end
    function M.Start()
    ...
    function M.IsAvailable() return getWeatherRemote() ~= nil end
    return M
end)()
...
CombinedModules.MerchantSystem = (function()
    ...
    pcall(function() MerchantUI = PlayerGui:FindFirstChild("Merchant") or PlayerGui:WaitForChild("Merchant", 3) end)
    local function OpenMerchant() if MerchantUI then MerchantUI.Enabled = true end end
    local function CloseMerchant() if MerchantUI then MerchantUI.Enabled = false end end
...
SellSection:Button({
    Title = "Sell All Now",
    Callback = function() AutoSellSystem.SellOnce() end,
})
...
SellSection:Input({
    ...
    Callback = function(value)
        ...
        if num then AutoSellSystem.Timer.SetInterval(num) AutoSellSystem.Count.SetTarget(num) end
    end,
})
...
SellSection:Toggle({
    ...
    Callback = function(state)
        ...
        if state then
            if autoSellMode == "Timer" then AutoSellSystem.Timer.Start() else AutoSellSystem.Count.Start() end
        else
...
MerchantSection:Button({
    Title = "Open Merchant",
    Callback = function() CombinedModules.MerchantSystem.Open() end,
})

MerchantSection:Button({
    Title = "Close Merchant",
    Callback = function() CombinedModules.MerchantSystem.Close() end,
})
...
local BuyCharm = (function()
    ...
    local function getPurchaseRemote() return EmbeddedEventResolver:GetRF("PurchaseCharm") end
    ...
    function M.SetCharmType(charmID) if charmID >= 1 and charmID <= 4 then M.Settings.CharmType = charmID return true end return false end
    function M.SetAmount(amount)
        amount = tonumber(amount)
        if amount and amount > 0 and amount <= 1000 then M.Settings.Amount = math.floor(amount) return true end
        return false
    end
    function M.SetDelay(delay)
        delay = tonumber(delay)
        if delay and delay >= 0 and delay <= 10 then M.Settings.Delay = delay return true end
        return false
    end
    ...
    function M.Stop() M.IsBuying = false M.AutoLoop = false return true end
    ...
    function M.DisableAutoLoop() M.AutoLoop = false M.IsBuying = false return true end
    function M.TestConnection() return getPurchaseRemote() ~= nil end
    return M
end)()
...
local AutoClaimPirateChest = (function()
    ...
    local function getClaimRemote() return EmbeddedEventResolver:GetRE("ClaimPirateChest") end
    ...
    function M.IsRunning() return enabled end
    return M
end)()
...
local AutoPotionSystem = (function()
    ...
    local function executeUsePotion()
        local ok, inventory = pcall(function() return Data:GetExpect({ "Inventory", "Potions" }) end)
...
local SkinSwapAnimation = (function()
    ...
    local SkinDatabase = {
        ["Default Rod"] = "rbxassetid://117319000848286",
        ...
        ["Blossom Conqueror"] = "rbxassetid://133255172354551",
        ["Fallen Staff"] = "rbxassetid://107412232735920",
        ["Blossom Guitar"] = "rbxassetid://132530630982956",
        ["Pirate Banjo"] = "rbxassetid://139089375187802",
        ["Undead Guitar"] = "rbxassetid://139089375187802",
    }
    ...
    local function findEnchantStoneUUIDs(enchantStoneItemId)
        ...
        pcall(function()
            for _, item in pairs(Data:GetExpect({"Inventory", "Items"})) do
                if item.Id == enchantStoneItemId then table.insert(uuids, item.UUID) end
            end
        end)
        return uuids
    end
...
local charmMap = { ["Bone Charm"] = 1, ["Algae Charm"] = 2, ["Magma Charm"] = 3, ["Clover Charm"] = 4 }
...
CharmSection:Toggle({
    ...
    Value = false,
    Callback = function(state)
        if state then BuyCharm.EnableAutoLoop() else BuyCharm.DisableAutoLoop() end
    end,
})
...
ClaimSection:Toggle({
    ...
    Value = false,
    Callback = function(state)
        if state then AutoClaimPirateChest.Start() else AutoClaimPirateChest.Stop() end
    end,
})
...
PotionSection:Input({
    ...
    Type = "Input",
    Placeholder = "30",
    Callback = function(value) AutoPotionSystem.SetInterval(value) end,
})
...
local skinNames = {
    "Default Rod", "Sunshine Cello", "Electric Guitar", "Soul Scythe",
    "Aether Monarch", "Dragonmaster Scythe", "Cloud Weaver", "World Tour Football",
    "Energy Blaster", "Butterfly Sword", "Galaxy Conqueror", "Overdrive",
    "Void Guitar", "Kitty Guitar", "Draconic Soul", "Divine Staff",
    "Empyrean Staff", "Golden Clockwork", "Bunny Summoner", "Easter Parasol",
    "Serpent's Trident", "Crimson Retribution", "Dark Matter Scythe", "Ethereal Sword",
    "Cupid's Harp", "Aurelian Bow", "Void Kraken", "Celestial Scythe",
    "Kitsune Greatsword", "Chromatic Katana", "Crescendo Scythe", "Blackhole Sword",
    "Eternal Flower", "Holy Trident", "Binary Edge", "The Vanquisher",
    "Frozen Krampus Scythe", "1x1x1x1 Ban Hammer", "Corruption Edge", "Gingerbread Katana",
    "Eclipse Katana", "Christmas Parasol", "Princess Parasol", "Blossom Conqueror",
    "Fallen Staff", "Blossom Guitar", "Pirate Banjo", "Undead Guitar"
}

local selectedSkin = "Default Rod"
...
SkinSection:Dropdown({
    ...
    Callback = function(selected)
        ...
        if SkinSwapAnimation.IsEnabled() and selected then
            SkinSwapAnimation.SwitchSkin(selected)
        end
    end,
})
...
EnchantSection:Dropdown({
    ...
    Value = 1,
    Callback = function(selected)
        if CombinedModules.AutoEnchant then CombinedModules.AutoEnchant.SetTargetEnchant(selected) end
    end,
})
...
EnchantSection:Toggle({
    ...
    Value = false,
    Callback = function(state)
        if state then CombinedModules.AutoEnchant.Start() else CombinedModules.AutoEnchant.Stop() end
    end,
})
...
TotemSection:Dropdown({
    ...
    Value = 1,
    Callback = function(selected) CombinedModules.AutoSpawnTotem.SetTotem(selected) end,
})
...
TotemSection:Toggle({
    ...
    Value = false,
    Callback = function(state)
        if state then CombinedModules.AutoSpawnTotem.Start() else CombinedModules.AutoSpawnTotem.Stop() end
    end,
})
...
CombinedModules.AutoTrade = (function()
    ...
    local RF_InitiateTrade = (EmbeddedEventResolver and EmbeddedEventResolver:GetRF("InitiateTrade")) or (Net and Net["RF/InitiateTrade"])
    local TierFish = { [1] = "Common", [2] = "Uncommon", [3] = "Rare", [4] = "Epic", [5] = "Legendary", [6] = "Mythic", [7] = "Secret" }
    local EnchantStoneIds = { ["Normal"] = 10, ["Double"] = 246, ["Evolved"] = 558 }
    local config = {
        enabled = false, targetPlayer = nil, tradeMode = "ByName", selectedItem = nil, itemAmount = 1,
        targetCoins = 0, selectedRarity = "Common", rarityAmount = 1, selectedStoneType = "Normal",
        stoneAmount = 1, currentTask = nil, playerManuallySelected = false,
    }
    local stats = {
        totalAttempted = 0, totalSuccess = 0, totalFailed = 0, targetAmount = 0,
        currentItem = "", status = "Idle", lastTradedItem = "", coinTraded = 0,
    }
    AutoTrade.OnStatsChanged = nil
    AutoTrade.OnCompleted = nil
    local function updateStats() if AutoTrade.OnStatsChanged then pcall(AutoTrade.OnStatsChanged, stats) end end
    local function setStatus(newStatus) stats.status = newStatus updateStats() end
    local function playerExists(playerName) return Players:FindFirstChild(playerName) ~= nil end
    ...
    local function executeTrade(playerName, itemUUID, itemName, coinValue)
        ...
        while attempts < 3 and config.enabled do
            local tradeSuccess = pcall(function() RF_InitiateTrade:InvokeServer(targetPlayer.UserId, itemUUID) end)
    ...
    local function chooseFishesByRange(fishList, targetCoins)
        ...
        if totalValue < targetCoins and #fishList > 0 then table.insert(selected, fishList[#fishList]) end
        ...
        for _, item in ipairs(inventory) do
            if not item.Favorited then
                ...
                if itemData and itemData.Data and itemData.Data.Type == "Fish" then
                    ...
                    if finalPrice > 0 then table.insert(fishList, { UUID = item.UUID, Name = itemData.Data.Name, Price = finalPrice }) end
                end
            end
        end
    ...
    local function autoTradeByEnchantStone()
        ...
        for i = 1, config.stoneAmount do
            ...
            local tradeSuccess, tradeResult = pcall(function() return RF_InitiateTrade:InvokeServer(targetPlayer.UserId, stoneUUID) end)
    ...
    function AutoTrade.SetTradeMode(mode)
        if mode == "ByName" or mode == "ByCoin" or mode == "ByRarity" or mode == "ByEnchantStone" then config.tradeMode = mode return true end
        return false
    end
...
PlayerSection:Button({
    ...
    Callback = function() playerDropdown:Set(CombinedModules.AutoTrade.GetPlayers()) end,
})
...
ByNameSection:Button({
    ...
    Callback = function() itemDropdown:Set(CombinedModules.AutoTrade.GetFishItems()) end,
})
...
ByNameSection:Input({
    ...
    Type = "Input",
    Placeholder = "1",
    Callback = function(value) CombinedModules.AutoTrade.SetItemAmount(value) end,
})
...
ByCoinSection:Input({
    ...
    Type = "Input",
    Placeholder = "0",
    Callback = function(value) CombinedModules.AutoTrade.SetTargetCoins(value) end,
})
...
ByRaritySection:Dropdown({
    ...
    Value = 1,
    Callback = function(value) CombinedModules.AutoTrade.SetRarity(value) end,
})
...
ByRaritySection:Input({
    ...
    Type = "Input",
    Placeholder = "1",
    Callback = function(value) CombinedModules.AutoTrade.SetRarityAmount(value) end,
})
...
ByStoneSection:Dropdown({
    ...
    Value = 1,
    Callback = function(value) CombinedModules.AutoTrade.SetStoneType(value) end,
})
...
ByStoneSection:Input({
    ...
    Type = "Input",
    Placeholder = "1",
    Callback = function(value) CombinedModules.AutoTrade.SetStoneAmount(value) end,
})
...
local WebhookModule = (function()
    ...
    M.FishConfig = { WebhookURL = "", DiscordUserID = "", HideIdentity = "", EnabledRarities = {} }
    M.DisconnectConfig = { WebhookURL = "", DiscordUserID = "", HideIdentity = "", Enabled = false }
    ...
    local TIER_COLORS = { [1] = 9807270, [2] = 3066993, [3] = 3447003, [4] = 10181046, [5] = 15844367, [6] = 16711680, [7] = 65535 }
    ...
    local function getPlayerDisplayName(config)
        if config.HideIdentity and config.HideIdentity ~= "" then return config.HideIdentity end
        return localPlayer.DisplayName or localPlayer.Name
    end

    local function getDiscordImageUrl(assetId)
        if not assetId then return nil end
        local thumbnailUrl = string.format("https://thumbnails.roblox.com/v1/assets?assetIds=%s&returnPolicy=PlaceHolder&size=420x420&format=Png&isCircular=false", tostring(assetId))
    ...
    local function sendFishWebhook(fish, meta, extra)
        ...
        if enabledRarities then
            ...
            for k, v in pairs(enabledRarities) do
                if type(k) == "number" and type(v) == "string" then raritySet[v] = true hasAnyFilter = true
                elseif type(k) == "string" and v == true then raritySet[k] = true hasAnyFilter = true end
            end
...
-- Buka tab pertama secara otomatis saat script dijalankan
Window:SelectTab(1)
